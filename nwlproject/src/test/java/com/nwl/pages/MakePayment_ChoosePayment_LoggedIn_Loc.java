...
public class MakePayment_ChoosePayment_LoggedIn_Loc extends BasePage {
	...
	@FindBy(xpath="//input[@class='other-amount']") WebElement txtChooseAmount;
	...
	String ccbUserName, ccbPwd, accountNo, scheduledPayment_CCB, payOffBal_CCB, CurrentBal_CCB, AmountDueOrInstalment, amount=null;
	
	public MakePayment_ChoosePayment_LoggedIn_Loc(WebDriver driver) throws Exception {
		PageFactory.initElements(driver, this);
		c=new Config(System.getProperty("user.dir")+"//src//test/resources//PropertyFiles//MakePayment_ChoosePayment_LoggedIn.properties");
	}

	public void navigateToMyBillsAndPaymentsPage() throws Exception {
		Thread.sleep(2000);
		tabMyBillsAndPayments.click();
		Thread.sleep(2000);
	}
	...
	public void userSelectsChooseAmountRadioButton(String amt) throws Exception {
		((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView(true);", radioChooseAmount);
		radioChooseAmount.click();
		elementVisible(100, txtChooseAmount);
		amount=amt;
		txtChooseAmount.sendKeys(amount);
		Thread.sleep(500);
		txtAmountDueOrInstalment_Amount.click();
		Thread.sleep(3000);
	}
	...
	public void entersCardDetails(String cardNo, String expiry, String securityCode) throws Exception {
	...
	public void verifyAccountFinancialHistoryInCCB() throws Exception {
		...
		if (CurrentBal_CCB.contains("‑")) {
			double abc = Double.parseDouble(CurrentBal_CCB.substring(2))+Double.parseDouble(amount);
			CurrentBalance="£‑"+String.valueOf(abc);
		}
		else if (!CurrentBal_CCB.contains("‑")) {
			double abc = Double.parseDouble(CurrentBal_CCB.substring(1))-Double.parseDouble(amount);
			CurrentBalance="£"+abc;
		}
			
		if (payOffBal_CCB.contains("‑")) {
			double xyz = Double.parseDouble(payOffBal_CCB.substring(2))+Double.parseDouble(amount);
			payOffBalance="£‑"+String.valueOf(xyz);
		}
		else if (!payOffBal_CCB.contains("‑")) {
			double xyz = Double.parseDouble(payOffBal_CCB.substring(1))-Double.parseDouble(amount);
			payOffBalance="£"+xyz;
		}
